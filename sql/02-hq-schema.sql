-- KodeDock HQ Internal Operating System Schema
...
CREATE TABLE IF NOT EXISTS hq_role_permissions (
    ...
    permissions JSONB NOT NULL DEFAULT '[]', -- Array of permission strings e.g., ["products.read", "products.approve"]
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(role_id)
);
...
CREATE TABLE IF NOT EXISTS hq_audit_logs (
    ...
    action VARCHAR(255) NOT NULL, -- e.g., 'PRODUCT_APPROVED', 'SELLER_SUSPENDED'
