...
export function ProductCard({
  ...
  if (badgeType === "best-seller" || (!badgeType && salesCount >= 20)) {
    ...
    badgeStyle = "bg-rose-500/20 text-rose-300 border-rose-500/40 shadow-rose-950/50";
  } else if (badgeType === "top-rated" || (!badgeType && rating >= 4.8 && reviews >= 5)) {
    badgeLabel = "TOP RATED";
    badgeStyle = "bg-amber-500/20 text-amber-300 border-amber-500/40 shadow-amber-950/50";
  } else if (badgeType === "new" || (!badgeType && salesCount < 5 && reviews === 0)) {
    badgeLabel = "NEW";
    badgeStyle = "bg-emerald-500/20 text-emerald-300 border-emerald-500/40 shadow-emerald-950/50";
  } else if (badgeType === "popular" || (!badgeType && salesCount >= 5)) {
    badgeLabel = "POPULAR";
    badgeStyle = "bg-violet-500/20 text-violet-300 border-violet-500/40 shadow-violet-950/50";
  }
  ...
  return (
    <Link href={`/products/${id}`} className="group h-full flex flex-col relative">
      <div className="bg-[#100e1c] rounded-[22px] p-3 border border-[#211d35] group-hover:border-violet-500/50 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.5)] group-hover:shadow-[0_8px_35px_-5px_rgba(139,92,246,0.25)] transition-all duration-300 h-full flex flex-col relative z-10 group-hover:-translate-y-1">
        {/* Preview Thumbnail Container */}
        <div className="relative aspect-[16/10] rounded-[16px] bg-[#161326] overflow-hidden mb-3.5 border border-[#26213d]">
          {image ? (
            <img
              src={image}
              alt={title}
            ...
            <div className="w-full h-full bg-gradient-to-tr from-[#131024] to-[#1e1936] flex items-center justify-center group-hover:scale-105 transition-transform duration-500">
          ...
          {badgeLabel && (
            <div
              className={`absolute top-2.5 left-2.5 text-[9px] font-black tracking-wider uppercase px-2.5 py-0.5 rounded-md border backdrop-blur-md shadow-md ${badgeStyle}`}
            >
              {badgeLabel}
            </div>
          )}
          ...
          {originalPrice && originalPrice > price && price > 0 && (
            <div className="absolute top-2.5 right-2.5 text-[9px] font-extrabold px-2 py-0.5 rounded-md bg-emerald-500 text-white shadow-md">
        ...
        <div className="px-1.5 flex-1 flex flex-col justify-between space-y-2.5">
          ...
          <div className="pt-3 border-t border-[#1e1b2f] flex items-center justify-between">
            ...
            <div className="flex items-baseline gap-1.5">
              {originalPrice && originalPrice > price && price > 0 && (
                <span className="text-[12px] font-semibold text-slate-500 line-through">
                  ₹{originalPrice}
                </span>
              )}
              <span
                className={`text-[16px] font-black tracking-tight ${
                  price === 0 ? "text-emerald-400" : "text-white"
                }`}
              >
                {price === 0 ? "Free" : `₹${price}`}
              </span>
