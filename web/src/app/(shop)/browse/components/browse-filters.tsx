          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 text-[13px] font-bold rounded-xl whitespace-nowrap shrink-0 transition-all duration-200 ${isActive
        ...
        <div className="flex items-center gap-2.5 shrink-0 self-end md:self-auto" ref={sortRef}>
          <div className="relative">
            ...
            >
              ...
              <ChevronDown
                className={`w-3.5 h-3.5 text-slate-500 dark:text-slate-400 transition-transform ${sortOpen ? "rotate-180" : ""
                  }`}
              />
            </button>
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-[#110f1c] rounded-2xl shadow-xl dark:shadow-[0_10px_30px_rgba(0,0,0,0.8)] border border-slate-200 dark:border-[#27233c] p-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                {SORT_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 text-[12px] font-semibold rounded-xl transition-colors ${(searchParams.get("sort") || "popular") === opt.value
