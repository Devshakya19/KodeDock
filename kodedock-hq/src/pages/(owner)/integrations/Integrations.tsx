...
import { Link2, Key, CheckCircle2, Eye, EyeOff, CreditCard, Cloud, Database, Mail, ShieldCheck, X } from "lucide-react";
...
const getProviderDetails = (provider: string) => {
  ...
  if (p.includes('github')) return { icon: Link2, color: "text-foreground", bg: "bg-secondary", desc: "OAuth authentication and repository sync." };
  if (p.includes('google')) return { icon: Link2, color: "text-foreground", bg: "bg-secondary", desc: "OAuth authentication and services." };
...
function IntegrationLogo({ provider, className }: { provider: string; className?: string }) {
  const p = provider.toLowerCase();
  ...
  if (error) {
    return (
      <div className={cn("flex items-center justify-center rounded-xl", details.bg, className)}>
  ...
  return (
    <div className={cn("flex items-center justify-center bg-secondary/10 rounded-xl p-3 border border-border/50", className)}>
      <img
...
export default function Integrations() {
  ...
  const selectedInteg = integrations.find(i => i.provider === selectedProvider);
  const selectedMergedConfig = selectedInteg ? { ...selectedInteg.config, ...(localConfigs[selectedInteg.provider] || {}) } : {};
  const isSaving = mutation.isPending && mutation.variables?.provider === selectedProvider;
    ...
    <div className="space-y-8 max-w-6xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    integ.is_active ? "border-primary/20" : "border-border opacity-80"
                  )}
                >
                  ...
                  <IntegrationLogo provider={integ.provider} className="w-20 h-20 mb-4 rounded-2xl bg-secondary/5 border-none" />
                  
                  <h3 className="text-lg font-bold text-foreground capitalize mt-2">{integ.provider}</h3>
                  <p className="text-xs text-muted-foreground font-medium text-center mt-2 line-clamp-2">{details.desc}</p>
                </button>
              );
            })}
          </div>
      ...
      {selectedInteg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setSelectedProvider(null)}></div>
          <div className="relative w-full max-w-xl bg-card border border-border rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-start justify-between p-6 border-b border-border bg-secondary/10">
              <div className="flex items-center gap-4">
                <IntegrationLogo provider={selectedInteg.provider} className="w-16 h-16 rounded-xl bg-background border border-border/50 shadow-sm" />
                <div>
                  <h3 className="text-2xl font-bold text-foreground capitalize">{selectedInteg.provider}</h3>
                  <p className="text-sm text-muted-foreground mt-1">{getProviderDetails(selectedInteg.provider).desc}</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto">
              <div className="flex items-center justify-between p-4 bg-secondary/20 rounded-xl border border-border mb-8">
                <div>
                  <h4 className="font-bold text-foreground">Enable Integration</h4>
                  <p className="text-xs text-muted-foreground mt-0.5">Turn this service on or off for your platform.</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input 
                    ...
                    onChange={(e) => handleToggle(selectedInteg, e.target.checked)}
                  />
              ...
              <div className="space-y-5">
                <h4 className="text-sm font-bold text-foreground border-b border-border pb-2">Configuration Secrets</h4>
                {Object.keys(selectedMergedConfig).length === 0 ? (
                  <p className="text-sm text-muted-foreground italic py-4">No configuration keys required for this integration.</p>
                ) : (
                  Object.keys(selectedMergedConfig).map(key => (
                    <ConfigInput
                      ...
                      onChange={(val) => updateLocalConfig(selectedInteg.provider, key, val)}
                    />
                  ))
                )}
              </div>
            ...
            <div className="p-6 border-t border-border bg-card flex justify-end gap-3 mt-auto">
              ...
              >
                {isSaving ? <CheckCircle2 className="w-4 h-4 animate-pulse" /> : "Save Changes"}
              </button>
            </div>
          </div>
        </div>
