  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 shadow-sm hover:border-primary/50 transition-all group">
      <div className="flex justify-between items-start mb-4">
        <div
          className={`h-10 w-10 rounded-lg ${bgClass} flex items-center justify-center transition-colors`}
        >
...
const PayoutActionCell = ({ request }: { request: PayoutRequest }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (status: string) => {
      setProcessing(true);
      await api.put(`/api/hq/finance/payouts/${request.id}/process`, {
        status,
        notes: `Processed manually via HQ`,
      });
    },
    onSuccess: () => {
  ...
  return (
    <div className="flex justify-end gap-2">
      <button
        onClick={() => {
          if (confirm("Reject this payout?")) mutation.mutate("REJECTED");
        }}
        disabled={processing}
      ...
      <button
        onClick={() => {
          if (confirm("Mark as Paid?")) mutation.mutate("PAID");
        }}
        disabled={processing}
...
const payoutColumns: ColumnDef<PayoutRequest>[] = [
  {
    ...
    cell: ({ row }) => (
      <span className="font-medium text-foreground">
        {row.original.seller_name || "Unknown Seller"}
      </span>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span className="font-bold text-foreground">
        {formatCurrency(row.original.amount_paise)}
      </span>
    ),
  },
  {
...
export default function Finance() {
  ...
  const { data: payoutsRes, isLoading: loadingPayouts } = useQuery({
    ...
    queryFn: async () =>
      await api.get("/api/hq/finance/payouts?status=PENDING"),
  });
    ...
    <div className="space-y-8 pb-20 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-muted-foreground mt-1 font-medium">
          Manage seller withdrawals and platform revenue.
        </p>
      </div>
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground font-medium">
              Platform GMV processed over last 7 days
            </p>
          </div>
        </div>
