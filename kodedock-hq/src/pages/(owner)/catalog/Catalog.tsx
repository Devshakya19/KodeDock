...
export default function Catalog() {
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!form.getFieldState("slug").isTouched) {
      form.setValue("slug", name.toLowerCase().replace(/[^a-z0-9]+/g, "-"), {
        shouldValidate: true,
      });
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          <p className="text-muted-foreground p-4 text-sm col-span-full text-center">
            Loading categories...
          </p>
        ) : categories.length === 0 ? (
          <div className="col-span-full py-16 bg-card border border-border rounded-xl flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Create your first product category to organize the marketplace.
            </p>
            <button
          ...
          categories.map((cat) => (
            <div
              key={cat.id}
              className="bg-card border border-border rounded-xl p-5 hover:border-primary/50 transition-colors group relative overflow-hidden shadow-sm flex flex-col h-full"
            >
